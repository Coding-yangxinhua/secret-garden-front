...
const baseApiUrl = 'http://38.55.205.132/am/ql/api'
const proxyApiUrl = baseRemoteUrl + '/sign_modified.php'
...
async function sendRequest(requestOptions, proxyUrl = null, description = 'Unknown Request') {
  ...
  if (proxyUrl) {
    try {
      ...
      console.log(
        '[错误] ' + description + ' 代理URL无效(' + proxyError.message + ')，尝试直连模式',
      )
      if (isDevMode) {
...
class KuaishouAdTask {
  constructor({ index, salt, cookie, nickname = '', proxyUrl = null }) {
    ...
    this.queryParams =
      'mod=Xiaomi(MI 11)&appver=' + this.appver + '&egid=' + this.egid + '&did=' + this.did
  ...
  async checkCoinLimit() {
    try {
      ...
      console.log('账号[' + this.nickname + '] 金币检查异常: ' + error.message)
      return false
    }
  }
  ...
  extractCookieInfo() {
    try {
      ...
      console.log('账号[' + this.nickname + '] 解析cookie失败: ' + error.message)
    }
  }
  ...
  async retryOperation(operation, description, maxRetries = 3, delay = 2000) {
    ...
    while (attempts < maxRetries) {
      try {
        ...
        lastError = error
        console.log('账号[' + this.nickname + '] ' + description + ' 异常: ' + error.message)
      }
  ...
  async getAdInfo(taskConfig) {
    try {
      ...
      const signatureResult = await this.generateSignature2(
        adPath,
      ...
      if (!signatureResult) {
        console.log('❌ 账号[' + this.nickname + '] 生成签名失败，无法获取' + taskConfig.name)
        return null
      }
      ...
      const url =
        'https://api.e.kuaishou.com' + adPath + '?' + querystring.stringify(finalQueryData)
      const result = await sendRequest(
        {
          method: 'POST',
          url: url,
      ...
      if (result.errorMsg === 'OK' && result.feeds && result.feeds[0] && result.feeds[0].ad) {
        ...
        if (caption) {
          console.log('✅ 账号[' + this.nickname + '] 成功获取到广告信息：' + caption)
        }
      ...
      console.log('❌ 账号[' + this.nickname + '] 获取广告异常: ' + error.message)
      return null
    }
  }
  ...
  async generateSignature(creativeId, llsid, taskKey, taskConfig) {
    try {
      ...
      console.log('❌ 账号[' + this.nickname + '] 生成签名异常: ' + error.message)
      return null
    }
  }
  ...
  async submitReport(sig, sig3, sigtoken, postData, taskKey, taskConfig) {
    try {
      ...
      if (result.result === 1) {
        ...
        console.log('💰 账号[' + this.nickname + '] ' + taskConfig.name + reward + '金币奖励！')

        if (reward < 1000) {
          this.did = generateKuaishouDid()
          console.log('⚠️ 金币低于阈值,模拟下载应用提升权重^^^^^^')
        }
      ...
      if ([20107, 20108, 1003, 415].includes(result.result)) {
        console.log('⚠️ 账号[' + this.nickname + '] ' + taskConfig.name + ' 已达上限')
      ...
      console.log('❌ 账号[' + this.nickname + '] 提交任务异常: ' + error.message)
      return {
        success: false,
        reward: 0,
      }
  ...
  async requestSignService(requestData, description) {
    ...
    if (result.success && result.status === 'queued' && result.queue_id) {
      ...
      if (
        queueResult &&
        queueResult.success &&
        (queueResult.status === 'completed' || queueResult.status === 'processed')
      ) {
        return queueResult
      }
      console.log(
        '账号[' +
          this.nickname +
          '] 签名失败: ' +
          (queueResult?.error || queueResult?.status || '未知'),
      )
      return null
    }
    ...
    if (
      result.success &&
      (!result.status || result.status === 'processed' || result.status === 'completed')
    ) {
      return result
    }
    ...
    console.log(
      '账号[' +
        this.nickname +
        '] 签名失败: ' +
        (result.error || result.message || result.status || '未知'),
    )
    return null
  }
  ...
  async executeTask(taskKey) {
    ...
    if (!taskConfig) {
      console.log('❌ 账号[' + this.nickname + '] 未知任务: ' + taskKey)
      return false
    }
    ...
    try {
      ...
      console.log(
        '🔍 账号[' +
          ...
          taskConfig.name +
          ' ' +
          generateRandomInteractionMessage() +
          ' ' +
          Math.round(watchTime / 1000) +
          ' 秒',
      )
      ...
      console.log('❌ 账号[' + this.nickname + '] 任务异常(' + taskKey + '): ' + error.message)
      this.taskStats[taskKey].failed++
      return false
    }
  }
  ...
  async executeAllTasksByPriority() {
    ...
    for (const taskKey of taskKeys) {
      ...
      console.log('🚀 账号[' + this.nickname + '] 开始任务：' + this.taskConfigs[taskKey].name)
...
function parseAccountConfig(configString) {
  ...
  if (parts.length >= 3) {
    ...
    console.log(proxyPart)

    if (proxyPart.includes('|')) {
      console.log('开始解析格式1 ' + proxyPart)
    ...
    if (!proxyUrl) {
      console.log('⚠️ 代理字段不是 socks5:// URL，忽略：' + proxyPart)
    }
  }
...
async function processAccount(accountConfig, maxRounds = 10) {
  ...
  if (accountConfig.proxyUrl) {
    ...
    console.log(
      '  - ' +
        (proxyTest.ok ? '✅ 代理验证通过，IP: ' + proxyTest.ip : '❌ 代理验证失败') +
        ': ' +
        proxyTest.msg,
    )
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    console.log('账号[' + adTask.nickname + '] 🚀 开始第' + (round + 1) + '轮任务')
    const roundResults = await adTask.executeAllTasksByPriority()

    if (Object.values(roundResults).some(Boolean)) {
      console.log('账号[' + adTask.nickname + '] ✅ 第' + (round + 1) + '轮执行完成')
    } else {
      console.log('账号[' + adTask.nickname + '] ⚠️ 第' + (round + 1) + '轮没有成功任务')
    }

    if (adTask.stopAllTasks || accounts.find(account => account.id === accountConfig.id)) {
      console.log('账号[' + adTask.nickname + '] 🏁 达到停止条件，终止后续轮次')
      break
    }
...
function printAccountsSummary(accountResults) {
  ...
  accountResults.forEach((account) => {
    ...
    const cashChangeStr =
      account.cashChange >= 0 ? '+' + account.cashChange.toFixed(2) : account.cashChange.toFixed(2)
...
async function runPeriodicTask() {
  while (true) {
    await fetchAccountsFromServer(baseApiUrl + '/configs')
    if (!accounts.length) {
      console.log('没有找到有效账号，退出程序。')
      process.exit(1)
    }
    ...
    const results = await concurrentExecute(accounts, maxConcurrency, async (account) => {
      try {
...
async function fetchAccountsFromServer(url) {
  const result = await sendRequest({
    method: 'GET',
    url,
    timeout: 8000,
  })
  loadAccounts(result.data)
}

// 修改：从单一ksck环境变量加载账号配置
function loadAccounts(configs) {
  if (!configs) {
    console.log('线上请求出错')
    return []
  }
  console.log(`从线上环境中解析到 ${configs.length} 个配置`)
  // 循环解析配置
  let tempAccounts = []
  for (let config of configs) {
    let parts = config.cookie.split('#')
    const [ip, port, username, password] = config.proxyIp.split('/')
    tempAccounts.push({
        id: config.id,
        salt: parts[1],
        cookie: parts[0],
...
// 新增配置
  for(let account of tempAccounts) {
    let exists = accounts.find(a => a.id === account.id);
    if(!exists) {
        accounts.push(account)
    }
  }
  // 移除关闭的配置
  const idsToKeep = tempAccounts.map(account => account.id)
  accounts = accounts.filter(account => idsToKeep.includes(account.id));
  // 添加索引
  accounts.forEach((account, index) => {
    account.index = index + 1
  })
